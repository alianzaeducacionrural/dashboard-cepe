...
- **Cronograma is directly clickable in admin mode**, for every producto (without actividades) and every actividad: clicking a cell **outside** the item's current `[mes_inicio, mes_fin]` extends the range to include that month (`extenderRangoCronograma()`); clicking a cell **inside** the range on a `tipo_medicion:'mensual'` item cycles its status (`ciclarMes()`); clicking an in-range cell on a non-mensual item shows a toast pointing to Panel admin to switch it to `'mensual'` first (`avisoCeldaNoMensual()` — deliberately non-destructive, never auto-converts or guesses monthly history from an existing manual/meta percentage). A producto **with** actividades keeps its consolidated (non-interactive) bar, same rule as elsewhere.
