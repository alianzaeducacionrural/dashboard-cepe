...
- All editing — objetivos, productos, actividades, MEL, **and instituciones** — create/edit/delete is centralized in a dedicated **Panel admin** tab (`viewAdmin()`, only in the tab bar when `ADMIN_MODE`; 5 sub-tabs, internal buttons via `.admin-subnav`), not scattered pencil buttons across public views — see "Panel admin" below. There is no public Instituciones tab anymore. Beneficiarios/Finanzas/Talento keep their original inline edit buttons (simpler flat data, unaffected by the tipo_medicion work).
- **Cronograma and product data are independent.** The Cronograma calendar is `estados_mensuales` alone (a JSON map `{"3":"realizado","5":"en_proceso"}`; a month key = scheduled, no key = not scheduled, gaps allowed; states `no_iniciado`/`en_proceso`/`atrasado`/`realizado`) and it applies to every producto/actividad regardless of `tipo_medicion`. In admin mode every cell is clickable and opens one modal (`abrirGestionMes()`) that only ever writes `estados_mensuales`. The Panel admin edit forms never read or write `estados_mensuales` and the Cronograma never writes `pct_real`/`tipo_medicion`/`meta` — editing one side cannot change the other. A producto **with** actividades has a non-interactive row whose per-month color is derived from its actividades that month (unanimous state wins, else most common, ties broken by urgency).
- **Admin session persists across page loads** (`guardarSesionAdmin`/`leerSesionAdmin`/`borrarSesionAdmin`, `localStorage` key `ucampo_admin_session`, mirrors the backend's 30 min token TTL) — `?admin=1` no longer re-prompts for the password if a still-valid session exists; exiting admin mode or letting the session expire clears it.
...
- **`docs/`** — four real source documents the original seed data was extracted from; **excluded from git** (see `.gitignore` — internal report/budget/PPTX files, more detail than the public dashboard shows). As of this writing the live Sheet's Objetivos/Productos/Actividades/Instituciones/MEL/Beneficiarios/Finanzas/Talento humano have been **intentionally reset at the user's request**: identity fields (nombre, descripcion, objetivo_id/producto_id relationships, institución names, MEL indicator names/periodicidad) were kept, but every figure/progress field was zeroed or blanked (pct_real, meta/alcanzado, estados_mensuales, valor_ejecutado, presupuestado/ejecutado, observaciones, estudiantes_2026/2027, etc.) so the user can re-populate everything manually through Panel admin. Do **not** treat `docs/` as reconciled against the live Sheet anymore, and do not re-seed from it without being asked. One direct consequence: every producto now defaults to `mes_inicio: mes_fin: 1`, which reads as overdue against `CONFIG.MES_ACTUAL` and paints the whole Productos/Cronograma view red until the user sets real dates (via Cronograma's click-to-extend or Panel admin) — this is correct given the rules, not a bug.
...
- **`computarProducto_(producto, mesActual, actividadesDelProducto)`** — when a producto has actividades, its `mes_inicio`/`mes_fin` become the min/max across all its actividades' computed ranges, and its `pct_real` becomes the average of their `pct_avance`. **Never edit a producto's own pct/dates/tipo_medicion once it has actividades** — those fields become derived and any edit to them would be silently overridden on the next read; the admin UI (`abrirEdicionProductoAdmin`) already accounts for this (edits are limited to `responsable`/`valor_ejecutado`/`observaciones` when actividades exist).
- **`peso` is always auto-computed, never stored** — `computarProductos_` (not `computarProducto_`, since it needs sibling counts) assigns `100 / (número de productos del mismo objetivo_id)` to every producto before enriching it, overwriting whatever the sheet's `peso` column holds. There is deliberately no `peso` input anywhere in the admin UI (create or edit) — adding one back would be misleading since any value typed there is discarded on the next read.
- **Three explicit measurement types**, chosen per producto/actividad via its `tipo_medicion` field (not inferred anymore, except for old rows missing the column, which still fall back to the pre-existing inference for backward compatibility):
  ...
  - **`'manual'`** for actividades is also available (`pct_real` as a plain reported number). There is no `'mensual'` type anymore: the month calendar is `estados_mensuales` and is independent of how the % is measured (old rows tagged `'mensual'` were migrated to `'manual'`; the backend treats any non-`meta`/`simple` type as `pct_real`). `mes_inicio`/`mes_fin` are derived as min/max of the scheduled months whenever `estados_mensuales` is non-empty.
...
Full CRUD surface, organized as 5 sub-tabs (internal `.admin-subnav` buttons, not top-level tabs) mirroring the backend services: **Objetivos** (create/edit/delete — originally edit-only since the program design fixes 3, but the user later asked for full CRUD to support building a program structure from scratch; deleting an objetivo cascade-deletes its productos and their actividades), **Productos** and **Actividades** (create/edit/delete; the edit modal's `tipo_medicion` select is `reactive:true` so choosing manual/meta/mensual re-renders the rest of the form via `camposMedicionProducto()`/`camposMedicionActividad()` — see "Form modal" below; `peso` is never a form field, see "peso is always auto-computed" above), **Indicadores MEL** (create/edit/delete), **Instituciones** (create/edit/delete — moved here from a public tab). Every write goes through the matching backend action (`createObjetivo`/`updateObjetivo`/`deleteObjetivo`, `createProducto`/`updateProducto`/`deleteProducto`, `createActividad`/`updateActividad`/`deleteActividad`, `createMEL`/`updateMEL`/`deleteMEL`, `createInstitucion`/`updateInstitucion`/`deleteInstitucion`) then `cargarDatos()` to refresh.
...
- Types: `text`/`number`/`textarea`/`checkbox`/`select` (options can be plain strings or `{value,label}` pairs — use the latter when the id and display text differ, e.g. objetivo/producto pickers).
